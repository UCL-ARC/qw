...
def get_repo_url(repo : git.Repo, name : str) -> str:
    ...
    if name is None:
        ...
        raise QwError(
            "No repo name supplied, and no remote called upstream or origin."
        )
    ...
    for remote in repo.remotes:
        if remote.url == repo:
            return repo
    raise QwError(
        "The supplied repo '{0}' is neither the name or url of a known remote."
        .format(repo)
    )
...
def get_service(conf : object=None) -> GitService:
    ...
    if name is None:
        raise QwError(
            "Configuration is corrupt. Please run `qw init`"
        )
    if name == str(Service.github):
        return Github(conf)
    raise QwError(
        "Do not know how to connect to the {0} service!"
        .format(name)
    )
...
def init(
    ...
    service : Annotated[Service, typer.Option(
    ...
    if os.path.isfile(qw_dir):
        raise QwError(
            ".qw file exists, which is blocking us from making a .qw directory. Please delete it!"
        )
    elif not os.path.isdir(qw_dir):
        os.mkdir(qw_dir)
    elif not force:
        raise QwError(
            ".qw directory already exists! Use use existing configuration or use --force flag to reinitialize!"
        )
    hur = remote_address_to_host_user_repo(repo)
    if hur is None:
        raise QwError(
            "'{0}' does not seem to be a valid repo url!"
            .format(repo)
        )
    host = hur[0]
    if service is None:
    ...
    if service is None:
        raise QwError(
            "'{0}' is not a service I know about!"
            .format(host)
        )
    conf = {
        'repo_url': repo,
